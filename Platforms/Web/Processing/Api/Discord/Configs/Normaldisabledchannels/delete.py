...
async def apiDiscordConfigsNormalDisabledChannelsDelete(cls:"PhaazebotWeb", WebRequest:ExtendedRequest) -> Response:
	"""
	...
	# get channel entry
	res_channel:list = await getDiscordServerNormalDisabledChannels(PhaazeDiscord, guild_id=guild_id, entry_id=entry_id)

	if not res_channel:
		return await cls.Tree.Api.Discord.Configs.Normaldisabledchannels.errors.apiDiscordConfigsNormalDisabledChannelNotExists(cls, WebRequest)
	...
	cls.BASE.Logger.debug(f"(API/Discord) Normal disabled channel: {guild_id=} deleted {entry_id=}", require="discord:configs")
	return cls.response(
