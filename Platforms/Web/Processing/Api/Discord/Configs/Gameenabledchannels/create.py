...
from .errors import apiDiscordConfigsGameEnabledChannelExists

async def apiDiscordConfigsGameEnabledChannelsCreate(cls:"WebIndex", WebRequest:Request) -> Response:
	"""
	...
	if res[0]["match"]:
			return await apiDiscordConfigsGameEnabledChannelExists(cls, WebRequest, channel_id=channel_id, channel_name=ActionChannel.name)
	...
	cls.Web.BASE.Logger.debug(f"(API/Discord) Game enabled channel: {guild_id=} added: {channel_id=}", require="discord:configs")
	return cls.response(
		text=json.dumps( dict(msg="Game enabled channel: Added new entry", entry=channel_id, status=200) ),
