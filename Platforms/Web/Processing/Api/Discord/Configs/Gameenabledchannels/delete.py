...
async def apiDiscordConfigsGameEnabledChannelsDelete(cls:"PhaazebotWeb", WebRequest:ExtendedRequest) -> Response:
	"""
	...
	# get channel entry
	res_channel:list = await getDiscordServerGameEnabledChannels(PhaazeDiscord, guild_id=guild_id, entry_id=entry_id)

	if not res_channel:
		return await cls.Tree.Api.Discord.Configs.Gameenabledchannels.errors.apiDiscordConfigsGameEnabledChannelNotExists(cls, WebRequest)
	...
	cls.BASE.Logger.debug(f"(API/Discord) Game enabled channel: {guild_id=} deleted {entry_id=}", require="discord:configs")
	return cls.response(
