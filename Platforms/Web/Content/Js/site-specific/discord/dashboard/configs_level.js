var ConfigsLevel = new (class {
	...
	load(x={}) {
		...
		.done(function (data) {
			...
			// insert current data
			insertData("[location=configs_level]", data.result);
	...
	edit(update) {
		...
		.done(function (data) {
			...
			insertData("[location=configs_level]", data.changes, true);
			Display.showMessage({content: data.msg, color:Display.color_success, time:1500});

		})
		.fail(function (data) {
			generalAPIErrorHandler( {data:data, msg:"error updating configs for level"} );
		})
	}
