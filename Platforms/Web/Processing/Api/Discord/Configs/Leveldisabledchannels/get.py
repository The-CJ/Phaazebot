...
async def apiDiscordConfigsLevelDisabledChannelsGet(cls:"PhaazebotWeb", WebRequest:ExtendedRequest) -> Response:
	"""
	...
	channel_res:list = await getDiscordServerLevelDisabledChannels(PhaazeDiscord, **Search.getAllTransform())

	result:Dict[str, Any] = dict(
			result=[Channel.toJSON() for Channel in channel_res],
			limit=Search["limit"],
			offset=Search["offset"],
			total=await getDiscordServerLevelDisabledChannels(PhaazeDiscord, count_mode=True, **Search.getAllTransform()),
			status=200
	)
