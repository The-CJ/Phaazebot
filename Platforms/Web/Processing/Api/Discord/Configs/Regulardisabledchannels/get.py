...
async def apiDiscordConfigsRegularDisabledChannelsGet(cls:"PhaazebotWeb", WebRequest:ExtendedRequest) -> Response:
	"""
	...
	channel_res:List[DiscordRegularDisabledChannel] = await getDiscordServerRegularDisabledChannels(PhaazeDiscord, **Search.getAllTransform())

	result:Dict[str, Any] = dict(
		result=[Channel.toJSON() for Channel in channel_res],
		limit=Search["limit"],
		offset=Search["offset"],
		total=await getDiscordServerRegularDisabledChannels(PhaazeDiscord, count_mode=True, **Search.getAllTransform()),
		status=200
	)
