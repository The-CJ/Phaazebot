...
async def apiDiscordConfigsBlacklistedWordsGet(cls:"PhaazebotWeb", WebRequest:ExtendedRequest) -> Response:
	"""
	...
	Search["word_id"] = Data.getStr("word_id", UNDEFINED, must_be_digit=True)
	Search["word"] = Data.getStr("word", UNDEFINED, len_min=1, len_max=128)
	...
	word_res:List[DiscordBlacklistedWord] = await getDiscordServerBlacklistedWords(PhaazeDiscord, **Search.getAllTransform())

	result:Dict[str, Any] = dict(
		result=[ARole.toJSON() for ARole in word_res],
		limit=Search["limit"],
		offset=Search["offset"],
		total=await getDiscordServerBlacklistedWords(PhaazeDiscord, count_mode=True, **Search.getAllTransform()),
		status=200
	)
