...
async def apiDiscordConfigsQuoteDisabledChannelsGet(cls:"PhaazebotWeb", WebRequest:ExtendedRequest) -> Response:
	"""
	...
	channel_res:List[DiscordQuoteDisabledChannel] = await getDiscordServerQuoteDisabledChannels(PhaazeDiscord, **Search.getAllTransform())

	result:Dict[str, Any] = dict(
		result=[ARole.toJSON() for ARole in channel_res],
		limit=Search["limit"],
		offset=Search["offset"],
		total=await getDiscordServerQuoteDisabledChannels(PhaazeDiscord, count_mode=True, **Search.getAllTransform()),
		status=200
	)
