...
from .errors import apiDiscordConfigsNormalDisabledChannelExists

async def apiDiscordConfigsNormalDisabledChannelsCreate(cls:"WebIndex", WebRequest:Request) -> Response:
	"""
	...
	if res[0]["match"]:
			return await apiDiscordConfigsNormalDisabledChannelExists(cls, WebRequest, channel_id=channel_id, channel_name=ActionChannel.name)
	...
	cls.Web.BASE.Logger.debug(f"(API/Discord) Normal disabled channel: {guild_id=} added: {channel_id=}", require="discord:configs")
	return cls.response(
		text=json.dumps( dict(msg="Normal disabled channel: Added new entry", entry=channel_id, status=200) ),
