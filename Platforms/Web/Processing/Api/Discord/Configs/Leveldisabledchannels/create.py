...
async def apiDiscordConfigsLevelDisabledChannelsCreate(cls:"PhaazebotWeb", WebRequest:ExtendedRequest) -> Response:
	"""
	...
	if res[0]["match"]:
		return await cls.Tree.Api.Discord.Configs.Leveldisabledchannels.errors.apiDiscordConfigsLevelDisabledChannelExists(cls, WebRequest, channel_id=ActionChannel.id, channel_name=ActionChannel.name)
	...
	cls.BASE.Logger.debug(f"(API/Discord) Level disabled channel: {Create['guild_id']=} added: {Create['channel_id']=}", require="discord:configs")
	return cls.response(
		text=json.dumps(dict(msg="Level disabled channel: Added new entry", entry=Create["channel_id"], status=200)),
