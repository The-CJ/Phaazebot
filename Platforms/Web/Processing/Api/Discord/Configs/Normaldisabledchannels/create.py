...
async def apiDiscordConfigsNormalDisabledChannelsCreate(cls:"PhaazebotWeb", WebRequest:ExtendedRequest) -> Response:
	"""
	Default url: /api/discord/configs/normaldisabledchannels/create
	"""
	...
	if not ActionChannel:
		return await cls.Tree.Api.Discord.errors.apiDiscordChannelNotFound(cls, WebRequest, channel_id=Create["guild_id"])
	...
	res:list = cls.BASE.PhaazeDB.selectQuery("""
		...
		WHERE `discord_disabled_normalchannel`.`guild_id` = %s
			AND `discord_disabled_normalchannel`.`channel_id` = %s""",
		(Create["guild_id"], Create["channel_id"])
	)

	if res[0]["match"]:
		return await cls.Tree.Api.Discord.Configs.Normaldisabledchannels.errors.apiDiscordConfigsNormalDisabledChannelExists(cls, WebRequest, channel_id=Create["channel_id"], channel_name=ActionChannel.name)
	...
	cls.BASE.Logger.debug(f"(API/Discord) Normal disabled channel: {Create['guild_id']=} added: {Create['channel_id']=}", require="discord:configs")
	return cls.response(
		text=json.dumps(dict(msg="Normal disabled channel: Added new entry", entry=Create["channel_id"], status=200)),
