...
class TwitchCommand(DBContentClass, APIClass):
	...
	async def increaseUse(self, cls:"PhaazebotTwitch", by:int=1) -> None:
		cls.BASE.PhaazeDB.query("""
			UPDATE `twitch_command`
			SET `uses` = `uses` + %s
			WHERE `id` = %s""",
			(by, self.command_id)
		)

		cls.BASE.Logger.debug(f"(Twitch) Increase command: C:{self.channel_id} C:{self.command_id} -> x{self.uses+by}", require="twitch:command")
