...
async def apiDiscordConfigsQuoteDisabledChannelsCreate(cls:"PhaazebotWeb", WebRequest:ExtendedRequest) -> Response:
	"""
	...
	if not ActionChannel:
		return await cls.Tree.Api.Discord.errors.apiDiscordMissingPermission(cls, WebRequest, channel_id=Create["channel_id"])
	...
	if res[0]["match"]:
		return await cls.Tree.Api.Discord.Configs.Quotedisabledchannels.errors.apiDiscordConfigsQuoteDisabledChannelExists(cls, WebRequest, channel_id=Create["channel_id"], channel_name=ActionChannel.name)
	...
	cls.BASE.Logger.debug(f"(API/Discord) Quote disabled channel: {Create['guild_id']=} added: {Create['channel_id']=}", require="discord:configs")
	return cls.response(
		text=json.dumps(dict(msg="Quote disabled channel: Added new entry", entry=Create["channel_id"], status=200)),
