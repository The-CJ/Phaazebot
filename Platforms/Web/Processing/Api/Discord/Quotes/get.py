...
from Platforms.Discord.db import getDiscordServerQuotes, getDiscordServerQuotesAmount
...
async def apiDiscordQuotesGet(cls:"WebIndex", WebRequest:Request) -> Response:
	...
	random:bool = Data.getBool("random", False)
	...
	# get quotes
	res_quotes:list = await getDiscordServerQuotes(PhaazeDiscord, guild_id=guild_id, quote_id=quote_id, random=random, limit=limit, offset=offset, content_contains=content_contains)
	...
	return cls.response(
		text=json.dumps( dict(
			...
			limit=limit,
			offset=offset,
			total=(await getDiscordServerQuotesAmount(PhaazeDiscord, guild_id)),
			status=200)
		),
