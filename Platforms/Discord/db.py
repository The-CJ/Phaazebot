...
async def getDiscordServerUsers(cls:"PhaazebotDiscord", guild_id:str, **search) -> List[DiscordUserStats]:
	...
	------------------
	* `member_id` Optional[int] : (Default: None)
	* `edited` Optional[int] : (Default: 0) [0=all, 1=not edited, 2=only edited]
	* `name` Optional[str] : (Default: None)
	* `order_str` Optional[str] : (Default: "ORDER BY id")

	Optional 'contains' keywords:
	-----------------------------
	* `name_contains` Optional[str]: (Default: None) [DB uses LIKE]
	...
	* `offset` `Optional[int] : (Default: 0)
	...
	Special:
	--------
	* `overwrite_sql` Optional[str] : (Default: None) [Overwrites autogenerated sql]
	* `overwrite_values` Optional[Union[tuple, dict]] : (Default: None) [Prepared values in `overwrite_sql`]
	"""
	# unpack
