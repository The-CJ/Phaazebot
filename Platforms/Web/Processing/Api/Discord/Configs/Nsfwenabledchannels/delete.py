...
async def apiDiscordConfigsNsfwEnabledChannelsDelete(cls:"PhaazebotWeb", WebRequest:ExtendedRequest) -> Response:
	"""
	...
	if not (CheckMember.guild_permissions.administrator or CheckMember.guild_permissions.manage_guild):
		return await cls.Tree.Api.errors.apiDiscordMissingPermission(cls, WebRequest, guild_id=guild_id, user_id=AuthDiscord.User.user_id)

	# get channel entry
	res_channel:List[DiscordNsfwEnabledChannel] = await getDiscordServerNsfwEnabledChannels(PhaazeDiscord, guild_id=guild_id, entry_id=entry_id)

	if not res_channel:
		return await cls.Tree.Api.Discord.Configs.Nsfwenabledchannels.errors.apiDiscordConfigsNsfwEnabledChannelNotExists(cls, WebRequest)
	...
	cls.BASE.Logger.debug(f"(API/Discord) Nsfw enabled channel: {guild_id=} deleted {entry_id=}", require="discord:configs")
	return cls.response(
